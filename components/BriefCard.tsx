    ...
    <div className={`bg-black/50 rounded-lg shadow-lg border ${isStale ? 'border-yellow/50' : 'border-white/10'}`}>
      <div className="p-4 border-b border-white/10 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          {icon}
          <h2 className="text-lg font-heading font-semibold text-grey">{title}</h2>
        </div>
        {isStale && (
            <div className="flex items-center space-x-2 text-yellow text-xs font-heading font-semibold bg-yellow/10 px-2 py-1 rounded-md">
