...
const ReasoningDisplay: React.FC<{ reasoning?: string; isNested?: boolean }> = ({ reasoning, isNested }) => {
  ...
  return (
    <div className={`${marginClass} p-3 bg-gray-800/70 border-l-4 border-cyan-600 rounded-r-md`}>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
         <span className="text-xs font-bold uppercase tracking-wider text-cyan-400 bg-gray-700 px-2 py-1 rounded-md">{item.level}</span>
      </div>
      
      <label className="block text-xs font-medium text-gray-400 mt-3 mb-1">Heading</label>
      <input
        type="text"
        className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-gray-200 font-semibold focus:ring-2 focus:ring-cyan-500"
        value={item.heading}
      ...
      <textarea
        rows={3}
        className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-gray-300 text-sm focus:ring-2 focus:ring-cyan-500"
    ...
    <div className="space-y-6">
       <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700">
         <div className="flex items-center mb-2">
            <ListTreeIcon className="h-6 w-6 mr-2 text-cyan-400" />
            <h2 className="text-lg font-semibold text-gray-200">Article Structure</h2>
        </div>
        ...
        <div>
          <label htmlFor="word_count" className="block text-sm font-medium text-gray-300 mb-1">Word Count Target</label>
          <input
            type="number"
            id="word_count"
            className="w-1/2 p-2 bg-gray-800 border border-gray-600 rounded-md text-gray-300 focus:ring-2 focus:ring-cyan-500"
        ...
        <div className="mt-6">
          <h3 className="text-md font-medium text-gray-300 mb-2">Content Outline</h3>
