...
// Helper to generate GFM-compatible slug for anchor links
...
const generateOutlineMarkdown = (outline: OutlineItem[], level = 2, isConcise: boolean): string => {
  ...
  outline.forEach(item => {
    ...
    if (item.additional_resources && item.additional_resources.length > 0) {
      markdown += `*   **Additional Resources Required:**\n`;
      markdown += item.additional_resources.map(r => `    *   ${r}`).join('\n') + '\n';
      hasBullets = true;
    }

    // Single blank line after all bullets (or after heading if no bullets)
    if (hasBullets) {
      markdown += '\n';
    }
...
const generateToc = (outline: OutlineItem[], level = 0): string => {
    ...
    outline.forEach(item => {
        ...
        toc += `${indent}- [${headingText}](#${createSlug(headingText)})\n`;
        if (item.children && item.children.length > 0) {
...
export const exportBriefToMarkdown = (
  ...
  // Header
  let markdownContent = `# ${mainKeyword} - Content Brief\n\n`;
  ...
  if (faqs?.questions?.length) {
      markdownContent += `- [Frequently Asked Questions](#h2-frequently-asked-questions)\n`;
  }
  markdownContent += `\n`;

  // Strategy and Goals
  markdownContent += `## Strategy and Goals\n\n`;
  markdownContent += `### Page Goal\n\n`;
  ...
  // Keyword Strategy
  markdownContent += `## Keyword Strategy\n\n`;
  if (keyword_strategy) {
  ...
  if (competitor_insights) {
    markdownContent += `## Competitive Analysis\n\n`;
  ...
  if (content_gap_analysis) {
    markdownContent += `## Content Gap Analysis\n\n`;
  ...
  // Article Structure
  markdownContent += `## Article Structure\n\n`;
  ...
  if (faqs && faqs.questions && faqs.questions.length > 0) {
      markdownContent += `## H2: Frequently Asked Questions\n\n`;
